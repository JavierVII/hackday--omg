// @ts-nocheck
// 移植自 React Bits（reactbits.dev）的 ImageTrail 组件，完整保留 variant 1-8。
// 类名以 image-trail 命名空间，避免与全局样式冲突。
...
class ImageItem {
  ...
  constructor(DOM_el) {
    this.DOM.el = DOM_el;
    this.DOM.inner = this.DOM.el.querySelector(".image-trail__img-inner");
    this.getRect();
    this.initEvents();
  }
  initEvents() {
...
class ImageTrailVariant1 {
  constructor(container) {
    ...
    this.images = [...this.DOM.el.querySelectorAll(".image-trail__img")].map((img) => new ImageItem(img));
    ...
    this._handlers.push(["mousemove", handlePointerMove]);
    this._handlers.push(["touchmove", handlePointerMove]);
    ...
    this._handlers.push(["mousemove", initRender]);
    this._handlers.push(["touchmove", initRender]);
...
export default function ImageTrail({ items = [], variant = 1 }: ImageTrailProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      for (const [type, fn] of instance._handlers ?? []) {
