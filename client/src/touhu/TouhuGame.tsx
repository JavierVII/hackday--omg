...
export function TouhuGame({ onClose, onComplete }: TouhuGameProps) {
  ...
  const next = () => { if (throws.length === totalThrows) { saveTouhuScore(throws); setPhase('summary') } else { setOutcome(undefined); startCycle() } }
  const closeGame = () => { if (onClose) onClose(); else setPhase('cover') }
  const evaluation = score >= 420 ? '月下神投' : score >= 280 ? '湖上雅士' : '投壶新秀'
  return <main className="touhu-shell"><div className="touhu-frame touhu-scene">
    ...
    <AssetSlot className="scene-background-slot" src={assetUrl("/assets/touhu/west-lake-night-bg.png")} alt="" /><div className="cloud cloud-a" /><div className="cloud cloud-b" /><div className="mountains" /><div className="pagoda"><i /><i /><i /><i /></div><div className="pavilion" />
    ...
    <aside className="scholar"><AssetSlot className="character-slot" src={assetUrl("/assets/touhu/character-west-lake.png")} alt="古风人物素材" /><div className="character-fallback"><span>月下雅士</span></div></aside><AssetSlot className="foreground-slot" src={assetUrl("/assets/touhu/foreground-lotus.png")} alt="" />
    <TouhuTarget outcome={outcome} glowing={phase === 'flying' && outcome !== 'MISS'} />
    {phase === 'flying' && <div className={`flying-token ${outcome === 'MISS' ? 'off-course' : ''}`}><img src={assetUrl("/assets/touhu/moon-token.png")} alt="月令" onError={event => { event.currentTarget.hidden = true }} /><div className="token-fallback">月</div><div className="trail" /></div>}
    ...
    {phase === 'summary' && <section className="summary-card"><p>五投已毕 · 西小湖</p><h1>{evaluation}</h1><div className="stats"><span><b>{score}</b>总分</span><span><b>{throws.filter(item => item.score).length}</b>命中</span><span><b>{throws.filter(item => item.grade === 'PERFECT').length}</b>PERFECT</span><span><b>{maxCombo}</b>最高连击</span></div><h2>Demo 模拟榜单</h2><ol>{[...rankSeed, { name: '西小湖', score, mine: true }].sort((a,b) => b.score-a.score).map((item, index) => <li className={'mine' in item ? 'mine' : ''} key={item.name}><span>{index + 1}</span><b>{item.name}</b><em>{item.score}</em></li>)}</ol><button className="gold-button reward-claim" onClick={() => onComplete?.(createTouhuGameResult(throws))}>领取本局奖励</button><button className="quiet-button" onClick={begin}>再投一局</button></section>}
  </div></main>
}
